...
You gotta read it in emacs to see the links.
...
Accepting the practice of writing a literate configuration means adapting to a more complicated workflow.
There is now an extra step to update your dotfiles.

Instead of editing your dotfiles directly
...now we must:
1. update the code in this document
2. run a command to export that code to the right place

*However*, we're gonna write a script to help us with that.
Also, we already have nearly that exact same extra step by accepting to use `chezmoi`, so it's no big deal.
...
**** Homebrew
MacPorts...? To use a Mac, we must `brew`.
