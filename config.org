...
After a major mactex upgrade, TeX Live regenerates [[https://www.tug.org/texlive/doc/fmtutil.html][format files]] (=fmtutil=) and font map files (=updmap=) on first run. This blocks any process that calls =pdflatex= synchronously (including Emacs during org export). See [[https://tug.org/texlive/Contents/live/texmf-dist/doc/texlive/texlive-en/texlive-en.html#x1-320003.4.4][TeX Live Guide §3.4.4: System font configuration]] and [[https://tug.org/texlive/scripts-sys-user.html][updmap/fmtutil sys vs user]] for background. The script below warms the cache after each mactex upgrade.

#+begin_src shell :tangle "./.chezmoiscripts/darwin/run_onchange_before_04-warm-tex-cache.sh.tmpl" :shebang "#!/bin/bash"
...
Config and snippets are tangled from [[#cfg-espanso][Cross-platform > Espanso]].

** Cross-platform

*** Espanso
:PROPERTIES:
:CUSTOM_ID: cfg-espanso
:END:
Cross-platform text expander.

**** Installation
Mac: [[#macos-espanso][macOS > Espanso]]

**** Config
#+begin_src yaml :tangle "./dot_config/espanso/config/default.yml"
...
**** Matches
#+begin_src yaml :tangle "./dot_config/espanso/match/base.yml"
...
*** Agent
Claude Code configuration, tangled to =~/.claude/=.

**** Instructions
Global agent instructions
#+begin_src markdown :tangle "./dot_claude/CLAUDE.md"
# Style
...
**** Skills
Skills are Claude Code agent instructions tangled to =~/.claude/skills/=.

***** tangle-moi
#+begin_src markdown :tangle "./dot_claude/skills/tangle-moi/SKILL.md"
---
name: tangle-moi
...
#+begin_src shell :tangle "./dot_claude/skills/tangle-moi/executable_ensure-dirs.sh" :shebang "#!/bin/sh"
...
#+begin_src shell :tangle "./dot_claude/skills/tangle-moi/executable_tangle.sh" :shebang "#!/bin/sh"
cd ~/.local/share/chezmoi && emacs --batch \
  --eval "(require 'org)" \
  --eval "(org-babel-tangle-file \"config.org\")"
#+end_src

#+begin_src shell :tangle "./dot_claude/skills/tangle-moi/executable_validate-analysis.sh" :shebang "#!/bin/sh"
...
***** commit
Single entry point for all commit work: splitting uncommitted changes across multiple commits, and rewriting existing commit history.

#+begin_src markdown :tangle "./dot_claude/skills/commit/SKILL.md"
---
name: commit
...
#+begin_src shell :tangle "./dot_claude/skills/commit/executable_snapshot.sh" :shebang "#!/bin/sh"
...
#+begin_src shell :tangle "./dot_claude/skills/commit/executable_stash.sh" :shebang "#!/bin/sh"
...
#+begin_src shell :tangle "./dot_claude/skills/commit/executable_restore.sh" :shebang "#!/bin/sh"
